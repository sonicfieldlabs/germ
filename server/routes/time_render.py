...
from server.schemas import GenerationResult, TimeClock, TimeRenderRequest, TimeRenderSource
...
def render_time(request: TimeRenderRequest) -> GenerationResult:
    ...
    with tempfile.TemporaryDirectory(prefix="germ_time_pitch_") as temp_root:
        ...
        for event in request.events:
            ...
            start_frame = max(0, min(_event_start_frame(event.tick, request.clock), output_frames))
    ...
    time_render_metadata = {
        ...
        "output_format": "wav",
        "output_channels": 2,
        "output_sample_rate": request.clock.sample_rate,
